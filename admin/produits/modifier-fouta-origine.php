<?php
/**
 * LA PAGE DE MODIFICATION D'ORIGINE DE FOUTA — MISE DE CÔTÉ, PAS SUPPRIMÉE.
 *
 * Le 23/08 au soir, modifier.php est devenu le portage du squelette de
 * fpl_natif/admin/piece-modifier.php (form-card : Identité / Emplacement /
 * Photos, modèles à cocher, génération au modèle unique). Ce fichier-ci est
 * la page telle qu'elle était juste avant, entière et fonctionnelle : on
 * l'ouvre à l'adresse modifier-fouta-origine.php et elle enregistre par le
 * même contrôleur (process_update_produit).
 *
 * NE PAS SUPPRIMER CE FICHIER tant que le nouveau modifier.php n'a pas
 * fait ses preuves à l'usage.
 *
...
<body class="page-produit-modifier">
    ...
    <div class="form-container">
        ...
        <div class="pm-sections">
            ...
            <?php /* LE NOM LIBRE DU FOURNISSEUR — le même champ qu'à l'ajout. Il ne
                     sert que si aucun fournisseur n'est choisi dans la liste. Sans
                     lui ici, modifier une pièce dont le fournisseur avait été saisi
                     à la main effaçait ce nom sans prévenir. */ ?>
            <?php if (produits_has_column('nom_fournisseur')): ?>
            <div class="form-group">
                ...
                <input type="text" id="nom_fournisseur" name="nom_fournisseur" maxlength="150"
                    ...
                    value="<?php echo htmlspecialchars((string) ($produit['nom_fournisseur'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>">
            ...
            <?php endif; ?>
            <?php /* LA RÉFÉRENCE D'ORIGINE ET LE CÔTÉ DE MONTAGE. Les deux se
                     saisissaient à la création et ne se corrigeaient nulle part :
                     une faute de frappe sur une référence OEM était définitive.
                     Mêmes champs qu'à l'ajout — même nom, même longueur, même
                     exemple — pour qu'une pièce se relise comme elle s'est écrite. */ ?>
            <?php if (produits_has_column('reference_oem') && pf_champ_visible('reference_oem')): ?>
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="reference_oem" name="reference_oem" maxlength="120"
                        ...
                        value="<?php echo htmlspecialchars((string) ($produit['reference_oem'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>">
                    <small class="form-hint">La référence d'origine du camion, gravée sur la pièce.</small>
                </div>
                <?php if (produits_has_column('position_montage') && pf_champ_visible('position_montage')): ?>
                <div class="form-group">
                    <label for="position_montage">Côté de montage</label>
                    <select id="position_montage" name="position_montage">
                        <option value="">— Indifférent —</option>
                        <?php $pos_actuelle = (string) ($produit['position_montage'] ?? ''); ?>
                        <?php foreach (['gauche' => 'Gauche', 'droite' => 'Droite'] as $pv => $pl): ?>
                        <option value="<?php echo $pv; ?>" <?php echo $pos_actuelle === $pv ? 'selected' : ''; ?>>
                            <?php echo $pl; ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
            ...
            ?>
            <?php /* LA TAILLE — elle se saisissait à la création et ne se corrigeait
                     nulle part. Pire : le formulaire envoyait « couleurs » sans
                     « taille », et le modèle écrit les deux ensemble — chaque
                     modification EFFAÇAIT donc la taille de la pièce. C'est pourquoi
                     aucune des 3 259 pièces n'en avait plus une seule.
                     Le champ est ici le même qu'à l'ajout : même nom, même longueur,
                     même exemple. */ ?>
            <?php if (produits_has_column('taille') && pf_champ_visible('taille')): ?>
            <div class="form-row">
                <div class="form-group">
                    <label for="taille">Taille</label>
                    <input type="text" id="taille" name="taille" maxlength="120"
                        placeholder="Ex. 4PK1250, 24V, 80 mm"
                        value="<?php echo htmlspecialchars((string) ($produit['taille'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>">
                    <small class="form-hint">Laissez vide si la pièce n'a pas de taille.</small>
                </div>
            </div>
            <?php endif; ?>
