<?php
/**
 * LA PAGE DES PIÈCES D'ORIGINE DE FOUTA — MISE DE CÔTÉ, PAS SUPPRIMÉE.
 *
 * Le 22/08, index.php est devenu le portage de la page de FPL natif. Ce
 * fichier-ci est la page telle qu'elle était juste avant, entière et
 * fonctionnelle : on l'ouvre à l'adresse index-fouta-origine.php.
 *
 * ON LA GARDE POUR Y REPRENDRE, UN PAR UN, LES APPORTS QUI N'EXISTENT PAS
 * CHEZ FPL NATIF et qu'il faudra replacer dans la nouvelle page :
 *   - la recherche en direct (ajax_live_search.php, admin-produits-index-search.js)
 *   - la galerie photos au clic sur la vignette (admin-produits-gallery-lightbox.js)
 *   - les filtres Marque et Fournisseur
 *   - le bouton « Suivi du catalogue »
 *   - le clic sur toute la ligne, et le fournisseur sous le nom
 *   - la carte « Toutes » et la carte « Retour » du bandeau
 *
 * NE PAS SUPPRIMER CE FICHIER tant que cette liste n'est pas soldée.
 *
...
/* LA RECHERCHE PRIME SUR LA NAVIGATION — règle de FPL natif. Chercher une
 * pièce, c'est chercher dans TOUT le catalogue : rester enfermé dans le rayon
 * qu'on parcourait ferait dire « aucun résultat » à une recherche qui en a. */
$fpl_recherche_prime = ($recherche !== '');
if ($fpl_recherche_prime) {
    $categorie_id = 0;
}
...
// défaut 5 : FPL natif appelle fpl_par_page('catalogue') sans second argument,
// et le défaut de la fonction vaut 5. Vérifié, pas supposé.
$per_page = fpl_par_page('catalogue_pieces', 5);
$page = max(1, (int) ($_GET['page'] ?? 1));
$total_produits = count_admin_produits_liste($categorie_id, $marque_id, $fournisseur_id, $sous_categorie_id, $du, $au, $recherche);
$total_pages = max(1, (int) ceil($total_produits / $per_page));
if ($page > $total_pages) {
    $page = $total_pages;
}
$offset = ($page - 1) * $per_page;
$produits = get_admin_produits_liste_paginated($categorie_id, $marque_id, $fournisseur_id, $offset, $per_page, $sous_categorie_id, $du, $au, $recherche);

$pagination_query_base = [];
// La recherche voyage avec la pagination : sans elle, la page 2 d'une
// recherche rendait le catalogue entier.
if ($recherche !== '') {
    $pagination_query_base['recherche'] = $recherche;
}
if ($categorie_id > 0) {
...
<body class="fpl-catalogue">
    ...
    <div class="page-produits-admin">
        <div class="content-header dashboard-hero page-produits-hero">
            ...
            <div class="dashboard-hero-text">
                ...
                <?php // DANS UN RAYON, FPL natif remplace le titre par le fil
                      // d'Ariane, EN HAUT : c'est lui qui dit où l'on se trouve,
                      // et il prend la place du titre au lieu de s'ajouter plus bas. ?>
                <nav class="fpl-fil fpl-fil--hero" aria-label="Fil d’Ariane">
                    <a href="index.php"><i class="fas fa-wrench" aria-hidden="true"></i> Pièces</a>
                    <span class="fpl-fil__sep" aria-hidden="true">›</span>
                    <?php if ($sous_categorie_id > 0): ?>
                    <a href="index.php?categorie_id=<?php echo (int) $categorie_id; ?>"><?php echo fpl_e($categorie_courante_nom); ?></a>
                    <span class="fpl-fil__sep" aria-hidden="true">›</span>
        ...
        <section class="produits-section page-produits-section" aria-labelledby="produits-section-heading"
            ...
            <?php // LE COMPTE DE PIÈCES A DESCENDU SOUS LES FILTRES : dans FPL
                  // natif il coiffe le TABLEAU, pas la page — il annonce ce que
                  // les filtres viennent de rendre, il doit donc venir après eux. ?>
            ...
            <form method="GET" action="" class="<?php echo htmlspecialchars($filtres_form_classes, ENT_QUOTES, 'UTF-8'); ?>"
                ...
                <div class="admin-filter-field page-produits-filters__categorie">
                    <label for="fpl-catfind">Catégorie</label>
                    <?php // ALLER DROIT À UNE CATÉGORIE — le geste de FPL natif. On
                          // tape deux lettres, les catégories ET les rayons qui
                          // correspondent apparaissent dessous, les flèches et
                          // Entrée y mènent. Une liste déroulante de toutes les
                          // catégories oblige à savoir où regarder ; ici on tape.
                          // La catégorie ouverte reste portée par le champ caché,
                          // pour qu'elle se combine aux dates et à la recherche. ?>
                    <input type="hidden" id="categorie_id" name="categorie_id" value="<?php echo (int) $categorie_id; ?>">
                    <div class="fpl-catfind">
                        <input type="text" id="fpl-catfind" autocomplete="off"
                            placeholder="<?php echo $categorie_courante_nom !== '' ? fpl_e($categorie_courante_nom) : 'Toutes'; ?>"
                            aria-label="Chercher une catégorie">
                        <div class="fpl-catfind-panel" id="fpl-catfind-panel" hidden></div>
                    </div>
                </div>
            ...
            <?php // LE TITRE DU TABLEAU, à la façon de FPL natif : le compte, et
                  // ce qui l'explique — « pour « joint » » quand on cherche,
                  // « dans Moteur » quand on parcourt. Un nombre seul ne dit pas
                  // de quoi il est le compte. À droite, le bouton d'ajout, qui
                  // n'apparaît que dans un rayon : c'est là qu'une pièce se range. ?>
            <div class="section-title page-produits-section__head">
                <h2 id="produits-section-heading"><?php echo (int) $total_produits; ?> pièce(s)<?php
                    if ($recherche !== '') {
                        echo ' pour « ' . fpl_e($recherche) . ' »';
                    } elseif ($sous_categorie_courante_nom !== '') {
                        echo ' dans ' . fpl_e($sous_categorie_courante_nom);
                    } elseif ($categorie_courante_nom !== '') {
                        echo ' dans ' . fpl_e($categorie_courante_nom);
                    }
                    ?>
                    <span class="page-produits-count" id="page-produits-count" hidden>(<?php echo (int) $total_produits; ?>)</span>
                </h2>
                <?php if ($sous_categorie_id > 0 && !admin_is_restricted_admin_account()): ?>
                <a href="ajouter.php?categorie_id=<?php echo (int) $categorie_id; ?>&amp;sous_categorie_id=<?php echo (int) $sous_categorie_id; ?>"
                   class="btn-primary page-produits-section__add">
            ...
            <?php if ($total_produits === 0): ?>
                <?php // LE VIDE NE SE DIT PAS D'UNE SEULE FAÇON — FPL natif a
                      // quatre phrases selon l'endroit où l'on se trouve. « Aucun
                      // produit à afficher » ne dit pas si l'on a mal cherché, si
                      // le rayon est vide, ou si le catalogue l'est. ?>
                <div class="empty-state page-produits-empty" id="page-produits-catalog-empty">
                    ...
                    <?php if ($recherche !== ''): ?>
                        <p class="page-produits-empty__title">Aucune pièce ne correspond à « <?php echo fpl_e($recherche); ?> »</p>
                        <p class="page-produits-empty__hint">Essayez un autre mot, une référence, ou <a href="index.php">tout effacer</a>.</p>
                    <?php elseif ($sous_categorie_id > 0): ?>
                        <p class="page-produits-empty__title">Aucune pièce ici</p>
                        <p class="page-produits-empty__hint">Ce rayon est vide<?php echo admin_is_restricted_admin_account() ? '.' : ' — ajoutez la première pièce.'; ?></p>
                    <?php elseif ($categorie_id > 0): ?>
                        <p class="page-produits-empty__title">Aucune pièce dans cette catégorie</p>
                        <p class="page-produits-empty__hint">Ouvrez une sous-catégorie ci-dessus pour en ajouter.</p>
                    <?php else: ?>
                        <p class="page-produits-empty__title">Le catalogue est vide</p>
                        <p class="page-produits-empty__hint">Élargissez la recherche, réinitialisez les filtres (catégorie, marque, fournisseur…) ou <a
                                href="index.php">tout effacer</a>.<?php echo admin_is_restricted_admin_account() ? '' : ' Vous pouvez aussi ajouter une pièce.'; ?></p>
                    <?php endif; ?>
                    <?php if (!admin_is_restricted_admin_account()): ?>
                        <?php if ($sous_categorie_id > 0): ?>
                        <a href="ajouter.php?categorie_id=<?php echo (int) $categorie_id; ?>&amp;sous_categorie_id=<?php echo (int) $sous_categorie_id; ?>" class="btn-primary page-produits-empty__cta">
                            <i class="fas fa-plus" aria-hidden="true"></i> Ajouter la première
                        </a>
                        <?php else: ?>
    ...
    <script>
    /* ALLER DROIT À UNE CATÉGORIE — repris de FPL natif.
       Deux lettres suffisent : les catégories et les rayons qui correspondent
       s'affichent sous le champ. Flèches pour parcourir, Entrée pour ouvrir,
       Échap pour refermer. Si le réseau ne répond pas, le champ reste inerte
       et le reste de la barre de filtres continue de marcher. */
    (function () {
        var champ = document.getElementById('fpl-catfind');
        var panneau = document.getElementById('fpl-catfind-panel');
        ...
        function dessiner(liste) {
            ...
            pos = -1;
            if (!liste.length) {
                panneau.innerHTML = '<div class="fpl-catfind-vide">Aucune catégorie ne correspond.</div>';
                panneau.hidden = false;
                return;
            }
            liste.forEach(function (c) {
                var a = document.createElement('a');
                a.className = 'fpl-catfind-item';
                a.href = c.url;
                a.innerHTML =
                    '<span class="fpl-catfind-ico"><i class="fas ' +
                        (c.sous ? 'fa-boxes-stacked' : 'fa-folder') + '"></i></span>' +
                    '<span><strong>' + echapper(c.name) + '</strong>' +
                        (c.parent ? ' <span class="fpl-catfind-parent">dans ' + echapper(c.parent) + '</span>' : '') +
                    '</span>';
        ...
        champ.addEventListener('input', function () {
            ...
            if (q.length < 2) { panneau.hidden = true; return; }
            minuteur = setTimeout(function () {
                fetch('ajax_recherche_categories.php?q=' + encodeURIComponent(q))
                    .then(function (r) { return r.ok ? r.json() : { categories: [] }; })
                    .then(function (d) { dessiner(d.categories || []); })
                    .catch(function () { /* réseau indisponible : on ne casse rien */ });
            }, 180);
        });

        champ.addEventListener('keydown', function (e) {
            var items = panneau.querySelectorAll('.fpl-catfind-item');
            if (panneau.hidden || !items.length) { return; }
            if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
                e.preventDefault();
                pos = (pos + (e.key === 'ArrowDown' ? 1 : -1) + items.length) % items.length;
                items.forEach(function (it, i) { it.classList.toggle('act', i === pos); });
            } else if (e.key === 'Enter') {
                e.preventDefault();
                (items[pos] || items[0]).click();
        ...
        document.addEventListener('click', function (e) {
            if (!e.target.closest('.fpl-catfind')) { panneau.hidden = true; }
        });
    })();
    </script>
