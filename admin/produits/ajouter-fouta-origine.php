<?php
/**
 * LA PAGE D'AJOUT D'ORIGINE DE FOUTA — MISE DE CÔTÉ, PAS SUPPRIMÉE.
 *
 * Le 23/08 au soir, ajouter.php est devenu le portage FIDÈLE du wizard de
 * FPL natif (piece-nouvelle.php : 4 étapes, récapitulatif, brouillons). Ce
 * fichier-ci est la page telle qu'elle était juste avant — le formulaire
 * Fouta enveloppé dans quatre onglets le 23/08 à midi —, entière et
 * fonctionnelle : on l'ouvre à l'adresse ajouter-fouta-origine.php et elle
 * enregistre toujours par le même contrôleur (process_add_produit).
 *
 * ON LA GARDE POUR Y REPRENDRE, SI BESOIN, CE QUE LE WIZARD N'A PAS REPRIS :
 *   - les sections « Variantes » et « Options d'achat » (poids / couleurs
 *     multiples) — déjà masquées sur l'ajout ici (classe
 *     admin-ajouter-produit-masquer), elles restent sur la modification ;
 *   - le choix libre de la catégorie / sous-catégorie en listes déroulantes
 *     quand on arrive sans contexte — le wizard, comme FPL natif, exige
 *     d'être entré par un rayon du catalogue.
 *
...
<body class="page-produit-modifier">
    ...
    <div class="contents-container pm-page">
        ...
        <div class="form-container">
            ...
            <form method="POST" action="" enctype="multipart/form-data" class="pm-form" id="form-produit-ajouter">
                ...
                <div class="pm-sections">
                    ...
                    <?php /* LE WIZARD EN 4 ÉTAPES de FPL natif. Les sections ne
                             bougent pas d'une ligne : elles sont simplement
                             enveloppées quatre par quatre. Un seul formulaire, un
                             seul envoi — les étapes ne sont qu'un déplacement à
                             l'écran, donc rien du chemin d'enregistrement ne change.
                             Sans JavaScript, les quatre panneaux restent visibles
                             les uns sous les autres : la page marche comme avant. */ ?>
                    <div class="wiz-steps" role="tablist" aria-label="Étapes">
                        <?php
                        $wiz_etapes = [
                            1 => 'Véhicule',
                            2 => 'La pièce',
                        ...
                        foreach ($wiz_etapes as $wiz_n => $wiz_titre) : ?>
                        <button type="button" class="wiz-step<?php echo $wiz_n === 1 ? ' active' : ''; ?>"
                                id="wiz-step-tab-<?php echo $wiz_n; ?>" data-step="<?php echo $wiz_n; ?>" role="tab">
                            <span class="wiz-step__num"><?php echo $wiz_n; ?></span>
                            <span class="wiz-step__nom"><?php echo htmlspecialchars($wiz_titre, ENT_QUOTES, 'UTF-8'); ?></span>
                        </button>
                        <?php endforeach; ?>
                    </div>
        ...
        <div class="pm-sticky-actions" role="contentinfo" aria-label="Enregistrement">
            <div class="pm-sticky-inner pm-sticky-inner--split">
                ...
                <?php /* LA NAVIGATION DU WIZARD, dans la barre du bas qui existait
                         déjà. « Enregistrer » ne se montre qu'à la dernière étape :
                         c'est ce qui distingue un wizard d'une page à ancres — on ne
                         valide pas une saisie qu'on n'a pas fini de faire.
                         Sans JavaScript, le compteur et les deux flèches restent
                         cachés et le bouton d'enregistrement reste visible. */ ?>
                <span class="wiz-counter" id="wiz-counter" hidden>Étape 1 sur 4</span>
                <button type="button" class="pm-btn-back" id="btn-prev" hidden>
                    <i class="fas fa-arrow-left" aria-hidden="true"></i> Précédent
                </button>
                <button type="button" class="pm-btn-primary btn-primary" id="btn-next" hidden>
                    Suivant <i class="fas fa-arrow-right" aria-hidden="true"></i>
                </button>
                <button type="submit" form="form-produit-ajouter" class="pm-btn-primary btn-primary" id="btn-submit">
    ...
    <script>
    /* LE WIZARD EN 4 ÉTAPES — repris de fpl_natif/admin/piece-nouvelle.php.
       Un seul formulaire, un seul envoi : les étapes ne sont qu'un déplacement
       à l'écran. Si ce script ne s'exécute pas, la classe « wiz-actif » n'est
       jamais posée et les quatre panneaux restent affichés les uns sous les
       autres — la page continue de marcher exactement comme avant. */
    (function () {
        var TOTAL = 4;
        var courant = 1;

        var panneau = function (n) { return document.getElementById('wiz-panel-' + n); };
        var onglet  = function (n) { return document.getElementById('wiz-step-tab-' + n); };
        var prev    = document.getElementById('btn-prev');
        var suiv    = document.getElementById('btn-next');
        var envoyer = document.getElementById('btn-submit');
        var compteur = document.getElementById('wiz-counter');
        var sections = document.querySelector('.pm-sections');

        if (!panneau(1) || !prev || !suiv || !envoyer || !sections) { return; }

        // C'est cette classe qui active le découpage : posée seulement ici.
        sections.classList.add('wiz-actif');
        prev.hidden = false;
        suiv.hidden = false;
        ...
        function allerA(n) {
            ...
            var avant = panneau(courant), ongletAvant = onglet(courant);
            if (avant) { avant.classList.remove('active'); }
            if (ongletAvant) {
                ongletAvant.classList.remove('active');
                if (n > courant) { ongletAvant.classList.add('done'); }
            }

            courant = n;

            var apres = panneau(courant), ongletApres = onglet(courant);
            if (apres) { apres.classList.add('active'); }
            if (ongletApres) { ongletApres.classList.add('active'); ongletApres.classList.remove('done'); }

            prev.style.display = courant > 1 ? '' : 'none';
            suiv.style.display = courant < TOTAL ? '' : 'none';
            // « Ajouter le produit » n'apparaît qu'à la dernière étape.
            envoyer.style.display = courant === TOTAL ? '' : 'none';
            compteur.textContent = 'Étape ' + courant + ' sur ' + TOTAL;

            window.scrollTo({ top: 0, behavior: 'smooth' });
        }

        /* LA SEULE VÉRIFICATION : le nom de la pièce, à l'étape 2. C'est celle
           de FPL natif — on ne bloque pas sur le reste, le serveur reste juge
        ...
        function etapeValide(n) {
            ...
            if (nom && !nom.value.trim()) {
                ...
                setTimeout(function () { nom.classList.remove('wiz-input-error'); }, 2000);
                return false;
            }
            return true;
        }
        ...
        suiv.addEventListener('click', function () {
            if (etapeValide(courant)) { allerA(courant + 1); }
        });
        prev.addEventListener('click', function () { allerA(courant - 1); });

        // Revenir sur une étape déjà franchie.
        document.querySelectorAll('.wiz-step').forEach(function (tab) {
            tab.addEventListener('click', function () {
                var n = parseInt(tab.dataset.step, 10);
                if (n < courant || tab.classList.contains('done')) { allerA(n); }
            });
        });

        /* Entrée ne doit pas envoyer le formulaire depuis une étape
           intermédiaire : elle fait avancer, comme un « Suivant ». */
        var form = document.getElementById('form-produit-ajouter');
        if (form) {
            form.addEventListener('keydown', function (e) {
                if (e.key !== 'Enter') { return; }
                var t = e.target;
                if (t && (t.tagName === 'TEXTAREA' || t.type === 'submit')) { return; }
                if (courant < TOTAL) {
                    e.preventDefault();
                    if (etapeValide(courant)) { allerA(courant + 1); }
                }
